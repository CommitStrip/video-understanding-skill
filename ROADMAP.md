...
> **2026-09-03 范围调整**：反无人机应用栈（原 `apps/anti_drone/`，含模型权重
> 与判别报告）与设备控制层（原 `devices/`）已从本仓库整体移除，回归独立的
> 反无人机仓库维护。本仓库自此聚焦视频理解核心（`vus/` + `bench/` + `tests/`）。
> 历史中相关路径与第三方工具遗留的作者身份已经 `git filter-repo` 重写清除。
...
- [ ] sherpa-onnx 真实字幕链路实测（已于 2026-09-03 完成：120 分钟课程视频，
  3505 段真实字幕，RTF 0.08）
