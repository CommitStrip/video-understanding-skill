...
| W5 ASR 清洗层 | `vus/asr_clean.py`：连叠折叠 + 相邻去重 + 英文幻觉标记（纯后处理，解码层不动） | 真实脏数据回测：课程 ASR 折叠 31 字冗余；干净数据零误杀 | [x] 完成于 2026-09-03 |
| W6 OCR 重构 Tier3 | `--ocr` 从 Tier2 逐关键帧移到管线后对 Tier3 代表帧定点执行（修复 25× 减速）；`vus/reconcile.py` ASR-OCR 跨模态 ocr_hint 标注 | 抖音复跑：11min 视频含 OCR 全程 94s（旧架构 6min 仅 33%） | [x] 完成于 2026-09-03 |
| W7 代表帧自适应 | `--max-reps N`：按时长推算 interval 迭代校正，LLM 消费场景推荐 `--max-reps 60` | 合成密集关键帧上预算约束满足 | [x] 完成于 2026-09-03 |

交付物：每波一个 tag（wave-0 … wave-4）+ 最终 `v0.2.0`。
后续迭代：v0.2.1（漂移软车道，见实战发现）→ **v0.3.0（W5-W7，ASR 清洗 + OCR Tier3 + 自适应选帧）**。
...
- **二轮根因（2026-09-03 人工核验 41 帧产物后发现两个覆盖缺口）**：
  "采纳即重置基准"的稀释效应——多页课件的持续单帧增量只剩硬阈的 0.9 倍
  （缺口内 9080 次检查 p50=9.0、0 次超阈），滑窗计数也无样本可数。
  追加**软车道**：软阈（硬阈×0.6）均值持续占满 `drift_soft_window_s`
  （30s）即采纳。真实缺口段复跑：缺口1 0→2 帧、缺口2 1→2 帧，全部闭合
...
- [x] Tier3 CLIP 语义去重与漂移视频的语义评估实测（PROTOCOL 已就绪；Trae 会话
  已用语义尺子实测 crv 对比：冗余度 12.00 vs 1.00）
- [x] sherpa-onnx 真实字幕链路实测（已于 2026-09-03 完成：120 分钟课程视频，
  3505 段真实字幕，RTF 0.08）
- [ ] 帧级对齐（当前 ASR-关键帧对齐为秒级桶）
- [ ] faster-whisper 可选 ASR 后端（竞品实测输出更干净，作为质量基准备选）
- [ ] Tier3 coverage 模式（事件定位/检索场景的全覆盖选帧，与理解抽样模式并存）
- [ ] 专名热词（解码层 contextual biasing；已裁决速度优先、交下游 LLM 补足，仅存档）
