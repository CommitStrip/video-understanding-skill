...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description='实时预算分配架构验证（生成测试视频并验证处理速率是否实时）')
    parser.add_argument('--out', default='./output/rt_validate',
                        help='输出目录（默认 ./output/rt_validate，相对当前工作目录）')
    ...
    write_json(out_dir, "rt_results.json",
               {"fast_scale": fast_scale, "keyframe_interval_hz": kf_hz,
                "all_realtime": all_ok, "results": results})

    print(f"\n结果已保存: {os.path.join(out_dir, 'rt_results.json')}")
